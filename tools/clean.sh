...
ansible -i ../invertory/hosts all -m shell -a 'rm -rf /opt/kubernetes /opt/etcd /opt/cni /etc/cni /var/run/flannel /var/lib/kubelet /etc/docker /var/lib/docker'
ansible -i ../invertory/hosts all -m shell -a "rm -f /usr/lib/systemd/system/{docker,flanneld,kubelet,kube-proxy,kube-apiserver,kube-controller-manager,kube-scheduler}.service"
ansible -i ../invertory/hosts all -m shell -a 'ip link set dev cni0 down; ip line set dev docker0 down'
ansible -i ../invertory/hosts all -m shell -a 'ip link delete cni0; ip link delete docker0'
